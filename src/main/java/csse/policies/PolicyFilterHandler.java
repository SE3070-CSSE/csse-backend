 ...
 */
@Component
class PolicyFilterHandler implements BeanDefinitionRegistryPostProcessor, EnvironmentAware {

    private static final Logger logger = LoggerFactory.getLogger(PolicyUrlMapping.class);
...
//
//        Set excludedRules = getActivePolicies();
//
//        logger.info(environment.getProperty("testval"));
//        logger.info(environment.getProperty("test"));
//
//
//        Arrays.stream(beanFactory.getBeanNamesForType(org.springframework.boot.web.servlet.FilterRegistrationBean.class)).forEach((String name) -> {
//            if(inHashSet(excludedRules, name)) {
//                logger.info("excluded filter : " + name);
//                return;
//            }
//            logger.info("inside post process : " + name);
//
